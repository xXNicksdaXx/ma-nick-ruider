...
The following subsections introduce the background of Software Product Lines, explain how feature trees encode relationships between features, and outline how the Universal Variability Language captures these ideas in a textual syntax.

% ----------------------------------------------------------
\subsection{Software Product Lines}
\label{subsec:bg_feature_modeling_spl}
% ----------------------------------------------------------

Software Product Lines (SPL) shift development from isolated systems to a reuse-oriented engineering approach~\cite{clements_software_2002}.
...
The process is typically divided into domain engineering and application engineering.
...
Application engineering then uses this model to derive specific products tailored to individual stakeholder requirements~\cite{buhne_modelling_2005, apel_overview_2009}.

Dynamic Software Product Lines (DSPL) extend these traditional SPL concepts by integrating both static variability in space and dynamic variability over time into a unified framework~\cite{burdek_staged_2014}.
Rather than completely shifting variability resolution away from design time, DSPL support multiple binding times to handle features across different operational modes.
This approach allows developers to clearly distinguish between static features finalized prior to product derivation and dynamic features left open as variation points.
These enable autonomous system adaptation and reconfiguration at runtime~\cite{capilla_overview_2014}.
...
\label{subsec:bg_feature_modeling_feature_trees}
% ----------------------------------------------------------

\gls{foda} establishes the foundation for hierarchical decomposition by defining features as user-visible characteristics identified through rigorous domain analysis~\cite{kang_feature-oriented_1990}.
...
\begin{itemize}
    ...
    A set of child features has an \textit{alternative} relationship with their parent feature when exactly one child is active in products that include the parent.
...
\begin{figure}
	...
	\caption{Example of a feature tree for a mobile phone, including cross-tree constraints. Taken from \cite{benavides_automated_2010}.}
...
Typical examples are \textit{requires} and \textit{excludes}.
...
More expressive formulations are also possible, including propositional constraints such as \texttt{A implies not B or not C}~\cite{galindo_automated_2019}.
...
\label{subsec:bg_feature_modeling_advancements}
% ----------------------------------------------------------

Advanced feature modeling extends the traditional binary logic of \gls{foda} by introducing quantitative constraints and metadata~\cite{benavides_automated_2010}.
...
\label{subsubsec:bg_feature_modeling_advancements_cardinality}
% ----------------------------------------------------------

This approach borrows from \gls{uml} multiplicities to define how often and how many instances of a feature may appear in a configuration~\cite{czarnecki_formalizing_2005, quinton_cardinality-based_2013}.
...
\label{subsubsec:bg_feature_modeling_advancements_attributes}
% ----------------------------------------------------------

Attribute-based modeling extends standard feature models by attaching additional information to features as attributes.
These extended models are often referred to as extended, advanced, or attribute-based feature models~\cite{benavides_automated_2010}.
...
Depending on the model, an attribute may store a numeric value, a boolean flag, or a string~\cite{benavides_automated_2010, karatas_attribute-based_2016}.
...
They also enable metric computation, such as calculating the total price of a selected configuration, by aggregating attribute values across all chosen features~\cite{benavides_automated_2010}.
In addition, attribute-based models support constraints over numeric and textual domains, enabling rules such as budget limits or consistency checks on string values~\cite{karatas_attribute-based_2016}.
...
The \gls{uvl} emerged to standardize these concepts into a unified textual exchange format, reducing fragmentation across modeling tools. 
Designed for both human readability and automated processing, \gls{uvl} uses indentation-based nesting to represent the tree hierarchy and provides a dedicated block for cross-tree constraints~\cite{sundermann_integration_2021, sundermann_yet_2021}. 
...
The Boolean level serves as the core language and can be easily encoded as a SAT problem.
The Arithmetic and Type levels introduce numeric constraints and typed features that require more complex reasoning engines, such as SMT solvers. 
Additionally, \gls{uvl} includes optional minor levels that support advanced constructs such as feature and group cardinalities, as well as string constraints~\cite{sundermann_yet_2021}.
...
\lstinputlisting[
    ...
    float,
    floatplacement=ht,
    caption={UVL specification of the mobile phone feature model shown in Figure~\ref{fig:mobile_phone_feature_tree}}, 
